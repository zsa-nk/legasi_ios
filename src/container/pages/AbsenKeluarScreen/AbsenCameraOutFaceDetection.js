...
const AbsenCameraOutFaceDetection = ({navigation}) => {
  ...
  const takePicture = debounce(async function(camera) {
    ...
    if(!netinfo.isConnected || !netinfo.isInternetReachable){
      ...
      Toast.showWithGravity('Presensi error Network unreachable', Toast.SHORT, Toast.CENTER); // error location
      return
    }
  ...
  const postPresensi = (fileUri) => {
    ...
    .then(json => {
      if (json.status_code === 200) {
        try {
          storeData(moment().format('DD-MM-YYYY') + '-SELFIE-OUT', 'SUDAH-SELFIE')
            ...
            return (
              ...
              { matchPercentage > 70 && takeSample == false ? (
                <View style={styles.buttonContainer}>
                  <TouchableOpacity onPress={() => takePicture(camera)} ref={takePictureRef}>
                    <Image style={styles.capture} source={require('../../../assets/icons/ic_camera.png')} />
                  </TouchableOpacity>
                </View>
              ): null}
