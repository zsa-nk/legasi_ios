...
const HistoryDiklat = ({navigation}) => {
  const {auth} = useContext(AuthContext);
  const [historyDiklat, setHistoryDiklat] = useState([]);
  ...
  const fetchData = () => {
    ...
    fetch_retry(
      ...
      .then(res => res.json())
      .then(json => {
        setHistoryDiklat(json.data);
        storeObjectData('dataDiklatUser', json.data);
      })
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView style={styles.contentWrapper}>
        ...
        <View style={styles.headerWrapper}>
          <View style={styles.header}>
            ...
            <View style={styles.headerTitleWrapper}>
              <Text style={styles.headerTitle}>Riwayat Diklat</Text>
        ...
        <View style={styles.app}>
          {historyDiklat.length > 0
            ? historyDiklat.map((data, index) => (
                <CollapsibleCard
                  key={index}
                  title={data.nama_diklat}
                  style={styles.collapsibleContentWrapper}>
                  <View style={{padding: 8}}>
                    <Text style={{padding: 5, fontSize: 18}}>
                      Jenis Diklat: {data.jenis_diklat}
                    </Text>
                    <Text style={{padding: 5, fontSize: 18}}>
                      Tanggal Diklat: {data.tgl_diklat}
                    </Text>
                    <Text style={{padding: 5, fontSize: 18}}>
                      Jumlah Jam Diklat: {data.jml_jam_diklat}
                    </Text>
                    <Text style={{padding: 5, fontSize: 18}}>
                      Penyelenggara Diklat: {data.penyelenggara_diklat}
                    </Text>
                  </View>
                </CollapsibleCard>
              ))
