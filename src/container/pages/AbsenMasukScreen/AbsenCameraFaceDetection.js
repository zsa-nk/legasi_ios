...
import { compareArrayOfContour, useWatchLocation, useHaversine, useGrantedPresensi} from '../../../app/_hooks';
...
const useLocalContour = () => {
  ...
  useEffect(() => {
    AsyncStorage.getItem('contours').then(result => {
...
const AbsenCameraFaceDetection = ({navigation}) => {
  ...
  const { grantedAbsent } = useGrantedPresensi(destination, currentPosition, diffPosition, distanceLimit);
  ...
  const takePicture = debounce(async (camera, tries = 5) => {
    ...
    if(!networkStatus.isInternetReachable || !networkStatus.isConnected){
      Toast.showWithGravity('Presensi error Network unreachable', Toast.SHORT, Toast.CENTER);
      return
    }
    ...
    if(granted && grantedAbsent){
      ...
      try {
        ...
        if(granted){
          dispatch({type: 'presensi/masuk.camera', data: { masuk: { file: data?.uri }}})
        ...
        // console.log(error);
        setProcessing(false)
        Toast.showWithGravity('Presensi error ' + error.message, Toast.SHORT, Toast.CENTER);
      }
    } else {
      checkPermission();
    }
    ...
    if(grantedAbsent){
      ...
      .then(json => {
        if (json?.status_code === 200) {
          storeData(moment().format('DD-MM-YYYY') + '-SELFIE-IN', 'SUDAH-SELFIE')
          storeData(moment().format('DD-MM-YYYY') + '-IN', 'TERCATAT')
          .then(_ => {
            Toast.showWithGravity('Presensi berhasil', Toast.SHORT, Toast.CENTER);
            navigation.navigate('RekapitulasiScreen');
          }).catch((err) => {
            console.log('error 3',err)
            Toast.showWithGravity('Presensi error' + err, Toast.SHORT, Toast.CENTER);
          });
        } else {
          // console.log(json)
          Toast.showWithGravity('Presensi gagal: ' + json.messages.error, Toast.SHORT, Toast.CENTER); // error location
          throw new Error(json.messages.error);
        }
      }).catch((err) => {
        console.log('error 4',err)
        Toast.showWithGravity('Presensi error' + err, Toast.SHORT, Toast.CENTER); // error location
      })
    }
  }

  useEffect(() => {
    if(cameraRef && (matchPercentage > 70) && (takeSample == false) && (postCount < 1)){
        ...
        {({camera, status}) => {
          if(status == 'READY'){
            return (
              <>
              <View>
                {arrayOfLabels.map((item, index) => (<Text key={index}>{item.text} - {item.confidence}</Text>))}
              </View>
